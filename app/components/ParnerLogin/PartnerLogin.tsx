      ...
      <section className=" pm-noise">
        <div className="container">
          <div className="text-center py-18 md:py-20 xl:py-30 2xl:pb-[120px] 2xl:pt-[90px]">
            <h2 className="font-light text-80 text-black heading">Partner Login</h2>
          </div>
        </div>
      </section>
          ...
          >
            <motion.form variants={container} className="w-full">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 2xl:gap-[70px]">
                {/* Field Example */}
                {[
                  { id: "companyName", label: "Company Name", value: companyname, setValue: setCompanyname },
                  { id: "licenseNumber", label: "Company License Number", value: licensenum, setValue: setLicensenum },
                  { id: "address", label: "Full Address *", value: address, setValue: setAddress, textarea: true, colSpan: true },
                  { id: "city", label: "City", value: city, setValue: setCity },
                  { id: "country", label: "Country", value: country, setValue: setCountry },
                  { id: "postal", label: "Postal Code *", value: postal, setValue: setPostal },
                  { id: "tax", label: "Tax Registration Number", value: tax, setValue: setTax },
                  { id: "phone", label: "Phone Number *", value: phone, setValue: setPhone },
                  { id: "mobile", label: "Mobile *", value: mobile, setValue: setMobile },
                  { id: "email", label: "Email ID *", value: email, setValue: setEmail },
                  { id: "emergency", label: "Emergency Contact *", value: emergency, setValue: setEmergency },
                ].map((field, i) => (
                  <motion.div
                    key={i}
                    variants={item}
                    className={`${field.colSpan ? "md:col-span-2" : ""} relative`}
                  >
                    {field.textarea ? (
                      <textarea
                        id={field.id}
