...
const DestinationSliderDemo: React.FC = () => {
  ...
  return (
    <section className="py-25 xl:py-30 overflow-hidden">
      ...
      <div className="relative pt-25 " ref={triggerRef}>
          ...
          <div ref={slidesContainerRef} className="flex h-full">
            {destinations.map((destination, index) => (
              <div key={destination.id} className={`slide flex-shrink-0 h-full relative transition-all duration-700 flex flex-col max-h-[792px]  ${index === currentSlide ? '' : 'mt-auto '}`} style={{ width: '80vw' }}>
                {/* Background Image */}
                <div className={`relative  flex flex-col items-end justify-end pb-5 pr-5 mr-25 xl:mr-[205px] transition-all duration-700 ${index === currentSlide ? 'h-[620px] order-1  ml-5 ' : 'h-[441px] order-2'}`}>
        ...
        <div className="absolute right-0 top-25 xl:top-30 transform z-50 pr-11 bg-white">
          {/* Progress lines */}
...
export default DestinationSliderDemo;