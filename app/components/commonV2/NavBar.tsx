...
//             {commonData.headerData.menuItems.map((item, i) => (
//               <li
//                 key={i}
...
//               >
//                 {/* If item has submenu, render span instead of link */}
//                 {item.submenu ? (
//                   <span className="cursor-pointer uppercase font-light w-full h-full leading-[1] text-base font-manrope text-black group-hover:text-white group-hover:scale-110 transition-all duration-300 ease-in-out ">
//                     {item.label}
//                   </span>
//                 ) : (
//                   <a
//                     href={item.href}
//                     className="uppercase font-light w-full h-full leading-[1] text-base font-manrope text-black group-hover:text-white group-hover:scale-110 transition-all duration-300 ease-in-out"
//                   >
//                     {item.label}
//                   </a>
//                 )}
...
//       {isOpen && (
//         <div
//           className="fixed top-0 left-0 w-full h-full bg-black opacity-50 z-40 transition-all duration-300 ease-in-out"
...
//                 href="#"
//                 target="_blank"
//                 className="rounded-full xl:w-[46px] xl:h-[46px] w-[36px] h-[36px] border border-black/22 hover:border-transparent flex items-center justify-center hover:bg-primary cursor-pointer transition-colors duration-300"
//               >
...
import { fetchMenuItems } from "@/lib/menuItemsV2";
...
const NavBar = ({
  ...
  // fetch dynamic menu items on mount
  useEffect(() => {
  ...
  return (
    <>
      {/* ===== Desktop Nav ===== */}
      <div className="w-fit rounded-3xl bg-white hidden lg:block z-10">
        <nav>
          <ul className="flex font-manrope">
            {menuItems.map((item, i) => (
              <li
                ...
                onMouseEnter={() => setHoveredMenu(item.label)}
                onMouseLeave={() => setHoveredMenu(null)}
                className="group text-19 font-sans relative last:rounded-r-full first:rounded-l-full hover:bg-primary transition-all duration-300 ease-in-out px-[17.5px] py-2 xl:py-[10.5px] first:pl-30px last:pr-30px"
              >
                {/* If item has submenu, render span instead of link */}
                {item.submenu && item.submenu.length > 0 ? (
                  <span className="cursor-pointer uppercase font-light w-full h-full leading-[1] text-19 text-black group-hover:text-white group-hover:scale-110 transition-all duration-300 ease-in-out">
                    {item.label}
                  </span>
                ) : (
                  <Link
