...
const AwardPage = () => {
    ...
    const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<AwardFormProps>();
    ...
    const handleAddAward = async (data: AwardFormProps) => {
        try {
    ...
    const fetchAwardData = async () => {
        try {
            ...
            console.log("Error in fetching award data", error);
        }
    }
    ...
    return (
        <div className='flex flex-col gap-5'>
            <form className='flex flex-col gap-5' onSubmit={handleSubmit(handleAddAward)}>
                        ...
                        <div key={field.id} className='grid grid-cols-1 gap-2 relative border-r pr-5 last:border-r-0'>
                            ...
                            <div className='flex flex-col gap-2'>
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <Input type='text' placeholder='Year' {...register(`firstSection.items.${index}.year`, {
                                        required: "Value is required"
                                    })} />
                                    {errors.firstSection?.items?.[index]?.year && <p className='text-red-500'>{errors.firstSection?.items?.[index]?.year.message}</p>}
                                </div>
                                <div className='flex flex-col gap-2'>
                ...
                <div className='flex justify-end mt-2'>
                        <Button type='button' addItem onClick={() => firstSectionAppend({ image: "", imageAlt: "",year:"",title:"" })}>Add Item</Button>
                    </div>
                </div>
                    </div>
